...
public class CloudVoiceRecordSearchAdapter extends BaseAdapter {
	 ...
	 * @param handler
	 */
	public CloudVoiceRecordSearchAdapter(Context context, Handler handler, List<CloudVoiceRecordEntry> cloudVoiceRecordEntries, ButtonClickListener buttonClickListener) {
	...
	public void modifySignedStatus(int position){
		...
		EventBus.getDefault().post(cvre.get(position));
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		title = (ViewGroup) convertView.findViewById(R.id.title);
		...
		tv_time.setText(entry.getCreate_time().substring(11, 16));
	...
	public void notifyListPlayIcon(int position, boolean isPlaying) {
		for (int i = 0; i < this.cvre.size(); i++) {
			cvre.get(i).setIsplaying(false);
		}
		this.cvre.get(position).setIsplaying(isPlaying);
		notifyDataSetChanged();
	}

	public void notifyListPlayIcon(boolean isPlaying) {
		for (int i = 0; i < this.cvre.size(); i++) {
			cvre.get(i).setIsplaying(false);
		}
		notifyDataSetChanged();
