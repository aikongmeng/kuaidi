...
/**
 * @ClassName: SmsRecordSearchAdapter 
 * @Description: 短信记录手机号或编号搜索界面
 * @author 顾冬冬
 * @date 2015-10-14 下午4:58:48
 */
public class SmsRecordSearchAdapter extends BaseAdapter {
	...
	public SmsRecordSearchAdapter(Context context, List<SmsRecord> smsRecords,OnclickListener clickListener) {
	...
	public void modifySignedStatus(int position){
		if(mSmsRecords.get(position).getSigned().equals("0")){
			mSmsRecords.get(position).setSigned("1");
		}else if(mSmsRecords.get(position).getSigned().equals("1")){
			mSmsRecords.get(position).setSigned("0");
		}
		EventBus.getDefault().post(mSmsRecords.get(position));
		notifyDataSetChanged();
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(R.layout.sms_record_adapter_item, null);
