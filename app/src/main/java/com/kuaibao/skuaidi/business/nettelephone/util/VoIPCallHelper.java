...
public class VoIPCallHelper implements OnMakeCallBackListener {
    ...
    private static String disPhoneNum;
    private VoIPCallHelper() {
        mVoIPCallback = new SubVoIPCallback();
        mUserInfo = new VoIPCallUserInfo();
        mUserInfo.setNickName(SkuaidiSpf.getLoginUser().getUserName());
        disPhoneNum=SkuaidiSpf.getLoginUser().getPhoneNumber();
        mUserInfo.setPhoneNumber(disPhoneNum);
    }
     ...
     * @param number 呼叫号码
     */
    public static String makeCall(ECVoIPCallManager.CallType callType , String number,String disNum) {
        disPhoneNum=disNum;
        initCall();
        if(getInstance().mCallInterface == null) {
            //KLog.d("make call error : ECVoIPCallManager null");
            return null;
        }
        if(getInstance().mCallSetInterface != null) {
            getInstance().mCallSetInterface.setVoIPCallUserInfo(getInstance().mUserInfo);
        }
        //Log.i("kb","呼叫参数:--->nickName:"+getInstance().mUserInfo.getNickName()+";显示号码:"+getInstance().mUserInfo.getPhoneNumber()+";呼叫号码:"+number);
        return getInstance().mCallInterface.makeCall(callType , number);
    }

    public static void makeCallBack(String master,String guest) {
        initCall();
        if(getInstance().mCallInterface == null) {
        	//KLog.d("make call error : ECVoIPCallManager null");
            UtilToolkit.showToast("数据错误,无法呼叫,请退出重试");
            return ;
        }
        ECVoIPCallManager.CallBackEntity callBackEntity =new ECVoIPCallManager.CallBackEntity();
        callBackEntity.caller=SkuaidiSpf.getLoginUser().getPhoneNumber();
        //callBackEntity.callerSerNum=guest;
        callBackEntity.called=guest;
        callBackEntity.calledSerNum=master;
    ...
    public static boolean getMute(){
        if(getInstance().mCallSetInterface == null) {
            //KLog.d("get mute error : CallSetInterface null");
            return false;
        }
    ...
    public static boolean getHandFree(){
        if(getInstance().mCallSetInterface == null) {
            //KLog.d("get hand free error : CallSetInterface null");
            return false;
        }
    ...
    public static void setMute() {
        initCall();
        if (getInstance().mCallSetInterface == null) {
            //KLog.d("set mute error : CallSetInterface null");
            return ;
        }
    ...
    public static void setHandFree() {
        ...
        initCall();
        if (getInstance().mCallInterface == null) {
            //KLog.d("set hand free error : CallSetInterface null");
            return ;
        }
    ...
    public static void releaseCall(String callId) {
        initCall();
        if (getInstance().mCallInterface == null) {
            //KLog.d("release call error : ECVoIPCallManager null");
            return;
        }
    ...
    public static void acceptCall(String callId) {
        initCall();
        if(getInstance().mCallInterface == null) {
            //KLog.d("accept call error : ECVoIPCallManager null");
            return ;
        }
    ...
    public static void rejectCall(String callId) {
        initCall();
        if(getInstance().mCallInterface == null) {
            //KLog.d("reject call error : ECVoIPCallManager null");
            return ;
        }
        // 3 主动拒接
    ...
    public static void initCall() {
        ...
        if(ourInstance.mCallSetInterface != null) {
            // 设置呼叫参数信息[呼叫昵称、呼叫手机号]
            VoIPCallUserInfo info = new VoIPCallUserInfo();
            info.setNickName(SkuaidiSpf.getLoginUser().getUserName());
            info.setPhoneNumber(disPhoneNum);
            ourInstance.mCallSetInterface.setVoIPCallUserInfo(info);
            getInstance().mUserInfo=info;
            //KLog.d("VOIPCallUserInfo：--->"+info.getNickName()+";"+info.getPhoneNumber());
        }
    }
