...
public class DaoBatchOperation implements BatchOperations{
	...
	View view;
	public DaoBatchOperation(Context context, EthreeInfoScanAdapter adapter, String scanType, ImageView iv,
			EditTextMaxLengthListener editTextMaxLengthListener, View view,List<NotifyInfo> upsite_list) {
	...
	public void operate() {
		...
		if (null == upsite_list || upsite_list.size() == 0) {
					...
					public void onClick(String... type) {
						for (int i = 0; i < array.size(); i++) {
							...
							if (info.isChecked()) {
								...
								/*
								 * info.setProblem_desc(type[type.length -
								 * 1]); if
								 * (!TextUtils.isEmpty(info.getPicPath())) {
								 * File file = new File(info.getPicPath());
								 * if (file.exists() == true) {
								 * file.delete(); } info.setPicPath(""); }
								 */
