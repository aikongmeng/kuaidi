...
public class PicFromPrintUtils {
	...
	/*************************************************************************
	 * width*height的图片，8个字节（8个像素点）是一个二进制，将二进制转化为十进制数值 y轴：24个像素点为一组，即width就是width/24组
	 * x轴：width个像素点（0-width） 里面的每一组（24*width），每8个像素点为一个二进制，（每组有3个，3*8=24）
	 **************************************************************************/
	/**
	 * 把一张Bitmap图片转化为打印机可以打印的bit(将图片压缩为width*height) 效率很高（相对于下面）
	 * 
	 * @param bit
	 * @return
	 */
	public static byte[] draw2PxPoint(Bitmap bit) {
		//System.out.println("width : "+bit.getWidth());
		byte[] data = new byte[(3 * (bit.getWidth() / 24) * bit.getWidth() + 5*(bit.getWidth() / 24))*(bit.getHeight()/24)];
		//System.out.println((byte) (int2ByteArr(bit.getWidth())[3] & 0xFF));
		//System.out.println((byte) (int2ByteArr(bit.getWidth())[0]));
		//byte[] data = new byte[30000];
		int k = 0;
		for (int j = 0; j < bit.getHeight() / 24; j++) {
			data[k++] = 0x1B;
			data[k++] = 0x2A;
			data[k++] = 33; // m=33时，选择24点双密度打印，分辨率达到200DPI。
			data[k++] = (byte) (int2ByteArr(bit.getWidth())[3] & 0xFF);
			data[k++] = int2ByteArr(bit.getWidth())[0];
	...
	public static Bitmap compressBitmap(Bitmap bitmapOrg, int newWidth,
		...
		// 加载需要操作的图片，这里是一张图片
		// Bitmap bitmapOrg =
		// BitmapFactory.decodeResource(getResources(),R.drawable.alipay);
		// 获取这个图片的宽和高
		int width = bitmapOrg.getWidth();
		int height = bitmapOrg.getHeight();
		// 计算缩放率，新尺寸除原始尺寸
	...
	public static Bitwise getBitwise(int i){
		byte[] arrB = new byte[4];
		arrB[0] = (byte) (i >> 24);
		arrB[1] = (byte) (i >> 16);
		arrB[2] = (byte) (i >> 8);
		arrB[3] = (byte) i;
		//System.out.println("H:"+arrB[3]);
		//System.out.println("L:"+(arrB[3] & 0xFF));
		return new Bitwise(arrB[3],arrB[0]);
	}
	
	public static class Bitwise{
		private byte lowFour;
		private byte highFour;
		
		public Bitwise(byte lowFour,byte highFour){
			this.highFour = highFour;
			this.lowFour = (byte) (lowFour & 0xFF);
		}
		
		public byte getLowFour() {
			return lowFour;
		}
		public void setLowFour(byte lowFour) {
			this.lowFour = lowFour;
		}
		public byte getHighFour() {
			return highFour;
		}
		public void setHighFour(byte highFour) {
			this.highFour = highFour;
		}
