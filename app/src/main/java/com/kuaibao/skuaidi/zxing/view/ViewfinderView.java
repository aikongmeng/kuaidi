...
package com.kuaibao.skuaidi.zxing.view;
...
public final class ViewfinderView extends View {
	...
	private int lineColor;// 扫描框中上下滑动的线
	...
	public ViewfinderView(Context context) {
		...
		fourCorners = resources.getColor(R.color.default_green_2);// 扫描框四的角的颜色
		lineColor = resources.getColor(R.color.red);
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			// paint.setTextSize(TEXT_SIZE_16 * density);
			// paint.setAlpha(0x95);
			// canvas.translate(20,80);
			// paint.setTypeface(Typeface.create("System", Typeface.BOLD));
			// canvas.drawText("请让客户展示他的微信或支付宝二维码，然后对准扫描", frame.left-50,
			// (float) (frame.bottom + (float) TEXT_PADDING_TOP * density),
			// paint);

			TextPaint textPaint = new TextPaint();
			textPaint.setColor(Color.WHITE);
			textPaint.setTextSize(TEXT_SIZE_16 * density);
			textPaint.setTypeface(Typeface.create("System", Typeface.BOLD));
			// 文字自动换行
			StaticLayout layout = new StaticLayout("请让客户展示他的微信或支付宝付款码，然后对准扫描", textPaint, frame.width() + 100,
					StaticLayout.Alignment.ALIGN_CENTER, 1.0F, 0.0F, true);
			canvas.save();
			textPaint.setTextAlign(Paint.Align.LEFT);
			canvas.translate(frame.left - 50, frame.bottom + (float) TEXT_PADDING_TOP * density);
			layout.draw(canvas);
			canvas.restore();

//			TextPaint textPaint2 = new TextPaint();
//			textPaint2.setColor(Color.WHITE);
//			textPaint2.setTextSize(TEXT_SIZE_12 * density);
//			textPaint2.setTypeface(Typeface.DEFAULT);
//			// 文字自动换行
//			StaticLayout layout2 = new StaticLayout("注：微信收款时，微信支付会收取4‰的手续费", textPaint2, frame.width() + 100,
//					StaticLayout.Alignment.ALIGN_CENTER, 1.0F, 0.0F, true);
//			canvas.save();
//			textPaint2.setTextAlign(Paint.Align.LEFT);
//			canvas.translate(frame.left - 50, height - (float) 50 * density);
//			layout2.draw(canvas);
//
//			canvas.restore();
